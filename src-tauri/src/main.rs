...
async fn main() -> Result<()> {
    ...
    tauri::Builder::default()
        .setup(|app| {
            ...
            setup_db(&resource_path).expect("error setting up database");
...
fn get_db_connection(window: &tauri::Window) -> Result<Connection, rusqlite_migration::Error> {
    let path = window
        .app_handle()
        .path_resolver()
        .resource_dir()
        .expect("could not get resource dir");
    let mut conn = Connection::open(path.join("encounters.db"))?;
    migrations::MIGRATIONS.to_latest(&mut conn)?;
    Ok(conn)
}

fn setup_db(resource_path: &Path) -> Result<(), rusqlite_migration::Error> {
    info!("setting up database");
    let mut conn = Connection::open(resource_path.join("encounters.db"))?;
    migrations::ensure_version_is_set(&conn)?;
    migrations::MIGRATIONS.to_latest(&mut conn)?;
